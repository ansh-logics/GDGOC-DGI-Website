...
type CreateFormState = CreateEventPayload & {
  hostName: string;
  hostAvatar: string;
  hostTitle: string;
  hostBio: string;
...
export default function AdminNewEventPage() {
  ...
  const handleChange = (
    ...
    value: string | CreateFormState["eventType"]
  ) => {
  ...
  const handleSpeakerChange = (
    index: number,
    field: keyof (CreateFormState["speakers"][number]),
    value: string
  ) => {
    setForm((prev) => {
  ...
  const handleAgendaChange = (
    index: number,
    field: keyof (CreateFormState["agenda"][number]),
    value: string
  ) => {
    setForm((prev) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: CreateEventPayload = {
        ...
        // Speakers are optional on create; only send ones with a non-empty name
        speakers: form.speakers.filter((s) => Boolean(s.name?.trim())),
        // Agenda items are optional; only send ones with a non-empty title
  ...
  const previewEvent: Main = useMemo(() => {
    ...
    const previewSpeakers = form.speakers
    ...
    const previewAgenda = form.agenda
      .filter((a) => Boolean(a.title?.trim()))
      .map((a) => ({
        time: a.time || "",
        title: a.title || "",
    ...
    return {
      ...
      host: {
        name:
          form.hostName ||
          user?.firstName ||
          "GDG on Campus Dronacharya",
        avatar:
          form.hostAvatar ||
          user?.profile_photo ||
          "/images/gdg-logo.png",
        role: form.hostTitle || "Organizer",
        email: user?.email || "",
        linkedin: form.hostLinkedin || "",
        x: form.hostX || "",
      },
        ...
        >
          <div>
            <button
              ...
              className="inline-flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white mb-3"
            >
              <svg
                className="w-4 h-4"
                fill="none"
                    ...
                    <div className="relative">
                      <select
                        ...
                        onChange={(e) =>
                          handleChange(
                            "eventType",
                            e.target.value as CreateFormState["eventType"]
                          )
                        }
                      >
